...
export function getStreakStats(store: ProgressStore): StreakStats {
  ...
  // Calculate current streak, counting back from the last completed day.
  // (Trailing days/weeks the user hasn't reached yet shouldn't count as breaks.)
  const lastCompletedIndex = sequence.lastIndexOf(true);
  let current = 0;
